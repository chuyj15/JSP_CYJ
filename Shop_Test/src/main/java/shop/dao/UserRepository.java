...
public class UserRepository extends JDBConnection {
	...
	public int insert(User user) {
		int result = 0;
		String sql = "INSERT INTO `user` (id, password, name, gender, birth, mail, phone, address, registDay)  VALUES (?,?,?,?,?,?,?,?,?)";
		try {
	...
	public int update(User user) {
		int result = 0;
		String sql = "UPDATE `user` SET name = ? , password = ?, gender = ?, birth = ? , mail = ?, phone = ?, address=? WHERE id = ?";
		try {
			psmt = con.prepareStatement(sql);
