...
<body>   
	...
	<div class="container shop p-5 mb-5" >
		...
		<!-- join_pro.jsp -->
			...
			<div class="d-grid gap-2 mt-5 mb-5 d-md-flex justify-content-md-between">
				...
				<!-- <input type="submit" class="btn btn-lg btn-primary" value="가입" onclick="validation()"/>  -->
				<input type="button" class="btn btn-lg btn-primary" value="가입" onclick="validation()"/> 
			</div>	
	...
	function validation(){
		...
		let pw = document.joinForm.pw.value;
		let pw_confirm  = document.joinForm.pw_confirm.value;
		let name = document.joinForm.name.value;
		//아이디: 영문자 또는 한글로 시작
		var regExpId = /^[a-z|A-Z|ㅏ-ㅣ|ㄱ-ㅎ|가-힣]/;
		if ( !regExpId.test(id) ){
		...
		//질문. 비밀번호 유효성검사를 어떻게 해야하는지 모르겠음..
		//var regExpPw = /^([a-zA-Z][~!@#$%^&*_+=-`,.<>/\;:])([a-zA-Z][~!@#$%^&*_+=-`,.<>/\;:])*[~!@#$%^&*_+=-`,.<>/\;:]+([a-zA-Z][~!@#$%^&*_+=-`,.<>/\;:])*$/;
		//var regExpPw = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[~!@#$%^&*_+=\-`,.<>/\;:]).{6,}$/;
		var regExpPw = /^(?=.*[a-zA-z])(?=.*[0-9])(?=.*[$`~!@$!%*#^?&\\(\\)\-_=+])(?!.*[^a-zA-z0-9$`~!@$!%*#^?&\\(\\)\-_=+]).{6,}$/;
		
		if ( !regExpPw.test(pw) ){
			alert("비밀번호는 영문자, 숫자, 특수문자(1개이상)를 사용해야합니다");
			return false;
		} 
		...
		//비번: 영문자, 숫자, 특수문자만 사용. 특수문자는 반드시 1개 포함하고 전체글자수가 6글자 이상
		//비번확인 : 비번값과 일치해야함
		if (pw != pw_confirm){
